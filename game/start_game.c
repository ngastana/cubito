...
/*   Updated: 2024/09/17 13:52:17 by ngastana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_start_game(t_game *cube)
{
	...
	ft_get_images(cube);
	cube->mlx->win_ptr = mlx_new_window(cube->mlx->mlx_ptr, cube->mlx->width, cube->mlx->height, "CUB3D | FPS: 0");
	cube->mlx->buffer = mlx_new_image(cube->mlx->mlx_ptr, cube->mlx->width, cube->mlx->height);
	cube->mlx->data_addr = mlx_get_data_addr(cube->mlx->buffer, &cube->mlx->bpp, &cube->mlx->size_line, &cube->mlx->endian);
